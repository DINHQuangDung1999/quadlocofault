    ...
    def __init__(
        ...
        for idx in range(len(output_channels)):
            ...
            if n == "none":
                ...
                layers.append(nn.LayerNorm([output_channels[idx], norm_input_dim]))
            else:
                raise ValueError(
        ...
        if global_pool == "none":
            ...
            layers.append(nn.AdaptiveMaxPool1d((1, 1)))
            last_dim = (1, 1)
        elif global_pool == "avg":
            layers.append(nn.AdaptiveAvgPool1d((1, 1)))
            last_dim = (1, 1)
        else:
            raise ValueError(
        ...
        if last_mlp_dim is not None:
            layers.append(nn.Linear(self._output_dim,last_mlp_dim))
...
def _compute_padding(input_length: int, kernel: int, stride: int, dilation: int) -> int:
    """Compute the optimal padding for the current layer.

    Reference: https://pytorch.org/docs/stable/generated/torch.nn.Conv1d.html
    """
    l = math.ceil((stride * math.floor(input_length / stride) - input_length - stride + dilation * (kernel - 1) + 1) / 2)
    return l
...
def _compute_output_dim(
    ...
    Reference: https://pytorch.org/docs/stable/generated/torch.nn.Conv1d.html
    """
    l = math.floor((input_length + 2 * padding - dilation * (kernel - 1) - 1) / stride + 1)
